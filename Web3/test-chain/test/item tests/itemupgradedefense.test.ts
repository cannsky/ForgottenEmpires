...
describe("Item New Item Test", () => {
    it("Tests item new item functionality", async () => {
        ...
        // TEST UPGRADE ITEM DEFENSE

        // Upgrade item defense of created item
        const tx2 = await appChain.transaction(alice, () => {
            item.upgradeDefense(UInt64.from(1));
        });
        // Sign the tx
        await tx2.sign();
        // Send the tx
        ...
        // Get item defense promise
        let itemDefensePromise = await appChain.query.runtime.Item.items.get(itemKey);
        // Get defense of the item created and upgraded for the player
        let itemDefense = await itemDefensePromise?.defense;
        // Expect block to be true
        expect(block2?.transactions[0].status.toBoolean()).toBe(true);
        // Expect item defense to be 2
        expect(itemDefense?.toBigInt()).toBe(2n);
    });
});