...
describe("Item New Item Test", () => {
    it("Tests item new item functionality", async () => {
        ...
        // TEST UPGRADE ITEM DAMAGE

        // Upgrade item damage of created item
        const tx2 = await appChain.transaction(alice, () => {
            item.upgradeDamage(UInt64.from(1));
        });
        // Sign the tx
        await tx2.sign();
        // Send the tx
        ...
        // Get item damage promise
        let itemDamagePromise = await appChain.query.runtime.Item.items.get(itemKey);
        // Get damage of the item created and upgraded for the player
        let itemDamage = await itemDamagePromise?.damage;
        // Expect block to be true
        expect(block2?.transactions[0].status.toBoolean()).toBe(true);
        // Expect item damage to be 2
        expect(itemDamage?.toBigInt()).toBe(2n);
    });
});