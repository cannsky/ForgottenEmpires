...
import { Item, ItemKey, EquipedItemKey } from "../../src/item";

import { log } from "@proto-kit/common";

log.setLevel("error");

describe("Item Equip Test", () => {
    it("Tests item equip functionality", async () => {
        // Define appchain
        ...
        // Equip newly created item to player
        const tx2 = await appChain.transaction(alice, () => {
            item.equipItem(UInt64.from(1), UInt64.from(1));
        });
        // Sign the tx
        await tx2.sign();
        // Send the tx
        ...
        // New equiped item key
        const equipedItemKey = new EquipedItemKey({
            owner: alice,
            slot: UInt64.from(1)
        });
        // Get equiped item promise
        let equipedItemPromise = await appChain.query.runtime.Item.equipedItems.get(equipedItemKey);
        // Get equiped item
        let equipedItem = await equipedItemPromise?.itemid;
        // Expect block to be true
        expect(block2?.transactions[0].status.toBoolean()).toBe(true);
        // Expect item type to be 1
        expect(equipedItem?.toBigInt()).toBe(1n);

        // Unequip item
        const tx3 = await appChain.transaction(alice, () => {
            item.unequipItem(UInt64.from(1));
        });
        // Sign the tx
        await tx3.sign();
        // Send the tx
        ...
        // Get equiped item promise
        let unequipedItemPromise = await appChain.query.runtime.Item.equipedItems.get(equipedItemKey);
        // Get equiped item
        let unequipedItem = await unequipedItemPromise?.itemid;
        // Expect block to be true
        expect(block3?.transactions[0].status.toBoolean()).toBe(true);
        // Expect item type to be 1
        expect(unequipedItem?.toBigInt()).toBe(0n);
    });
});