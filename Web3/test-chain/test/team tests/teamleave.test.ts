...
describe("Team Leave Team Test", () => {
    it("Tests team leave team functionality", async () => {
        ...
        // Get promise
        let playerTeamPromise = await appChain.query.runtime.Team.playerInvitations.get(teamInvitation);
        // Get invited player's invitation
        let playerTeam = await playerTeamPromise?.active;
        // Expect block to be true
        expect(block3?.transactions[0].status.toBoolean()).toBe(true);
        // Expect invitation value to be 0
        ...
        // Get promise
        let teamPromise = await appChain.query.runtime.Team.teams.get(UInt64.from(1));
        // Get player team member count
        let newTeamMemberCount = teamPromise?.memberCount;
        // Expect block to be true
        expect(block4?.transactions[0].status.toBoolean()).toBe(true);
        // Expect player team member count value to be 0
        expect(newTeamMemberCount?.toBigInt()).toBe(1n);
    });
});