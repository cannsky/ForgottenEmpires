...
describe("Rune Upgrade Fire Rune Test", () => {
    it("Tests rune upgrade fire rune functionality", async () => {
        // Define appchain
        ...
        // Upgrade fire rune of the player
        const tx2 = await appChain.transaction(alice, () => {
            rune.upgradeFireRune();
        });
        // Sign the tx
        await tx2.sign();
        // Send the tx
        ...
        // Get max level
        let fireLevel = await appChain.query.runtime.Rune.playerRunes.get(alice).value.firelevel;
        // Expect block to be true
        expect(block2?.transactions[0].status.toBoolean()).toBe(true);
        // Expect fire level to be 2
        expect(fireLevel?.toBigInt()).toBe(2n);
    });
});